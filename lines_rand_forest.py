...
def main():
    ...
    test_rf(train_x, train_y, test_x, test_y)

    rf = load('pck_nets/lines_rf.pck')
    valid_preds = rf.predict(test_x)
...
def test_rf(train_x, train_y, test_x, test_y):
    ...
    valid_preds = rf.predict(test_x)
    print(metrics.classification_report(test_y, valid_preds))
    save(rf, 'pck_nets/lines_rf.pck')


def save(forest, file_name):
    with open(file_name, 'wb') as fp:
        cPickle.dump(forest, fp)


def load(file_name):
    with open(file_name, 'rb') as fp:
        forest = cPickle.load(fp)
    return forest
